...
import { Users, UserPlus, Edit2, Trash2, Mail, ShieldCheck, ShieldAlert, AlertTriangle, Search, X, Clock } from "lucide-react";
                    ...
                    <table className="min-w-full divide-y divide-slate-100">
                        <thead className="bg-slate-50">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Riwayat</th>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            {!user.can_delete ? (
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${
                                                    ...
                                                    {user.role === 'superadmin' ? 'Protected' : 'Ada Riwayat'}
                                                </span>
                                            ) : (
                                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-slate-50 text-slate-400 border border-slate-200">
                                                    Tidak ada
                                                </span>
                                            )}
                                        </td>
                                            ...
                                            <div className="flex items-center justify-center gap-2">
                                                ...
                                                {!user.can_delete ? (
                                                    <span
                                                        ...
                                                        title={user.delete_reason || 'Tidak dapat dihapus'}
                                                    >
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
                        ...
                        <p className="text-sm text-slate-500 mb-8">
                            Akses login untuk <span className="font-bold text-slate-700">{confirmDelete.name}</span> akan dihapus selamanya. Tindakan ini tidak dapat dibatalkan.
                        </p>
                        <div className="flex gap-3">
                            <button onClick={() => setConfirmDelete({ show: false, id: null, name: "", hasActivity: false })}
