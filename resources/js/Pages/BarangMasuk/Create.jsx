...
import { Plus, Trash2, Save, Upload, Package, ArrowRight, ClipboardList, FileSpreadsheet, AlertCircle, X, Import } from "lucide-react";
      ...
      reader.onload = (event) => {
        try {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            const identifier = cols[0]; // Bisa nama_barang atau kode_barang
            ...
            if (i === 1) {
              if (supplierName) {
                const sFound = suppliers.find(s => s.nama_supplier.toLowerCase().includes(supplierName.toLowerCase()) || supplierName.toLowerCase().includes(s.nama_supplier.toLowerCase()));
                if (sFound) importedSupplierId = sFound.id;
              }
              if (tanggal) {
          ...
          if (newItems.length > 0) {
            setData({
              ...data,
              items: newItems,
              supplier_id: importedSupplierId,
              tanggal_masuk: importedDate
            });
            alert(`Berhasil memuat ${newItems.length} item ke dalam form.`);
          } else {
            alert("Tidak ada kode barang yang cocok ditemukan dalam file CSV. Mohon pastikan kode barang (A1, A2...) sudah sesuai.");
          }
        } catch (err) {
          console.error(err);
          alert("Terjadi kesalahan saat membaca file. Pastikan format CSV benar.");
        } finally {
          setIsImporting(false);
          e.target.value = null;
        }
    ...
    <div className="max-w-5xl mx-auto pb-20">
      <div className="mb-8 flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 font-medium italic">Catat kedatangan stok baru dari supplier secara massal atau import via CSV.</p>
        </div>
      ...
      <div className="mb-8 bg-emerald-50/50 border-2 border-dashed border-emerald-200 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <a
            href={route('barang-masuk.template')}
